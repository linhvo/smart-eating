...
if (Ember.ResourceAdapter === undefined) {
    Ember.ResourceAdapter = Ember.Mixin.create({
        ...
        _resourceRequest: function(params) {
            params.url = (params.url || this._resourceUrl()) + ".json";
...
Ember.Resource = Ember.Object.extend(Ember.ResourceAdapter, Ember.Copyable, {
    ...
    deserialize: function(json) {
        ...
        var r = json[this.resourceName];
        for(var prop in r) {
            if (r.hasOwnProperty(prop)) this.deserializeProperty(prop, r[prop]);
        }
    ...
    deserializeProperty: function(prop, value) {
        //todo CUSTOMIZATION: convert a ruby-like property name, e.g. my_string to camelCase
//        var camelCased = prop.replace(/_([a-z])/g, function (g) { return g[1].toUpperCase() })
        this.set(prop, value);
    },
    ...
    _resourceUrl: function() {
        var url = this.resourceUrl || this.get('resourceUrl'),
            id = this._resourceId();
...
Ember.ResourceController = Ember.ArrayController.extend(Ember.ResourceAdapter, {
     ...
     */
    _resourceUrl: function() {
        if (this.resourceUrl === undefined && this.get('resourceUrl') === undefined) {
        ...
        return this.resourceUrl || this.get('resourceUrl');
    }
});